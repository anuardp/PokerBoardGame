...
[TestFixture]
public class Game_DrawCardFromTop
{
    private Game _game;
    [SetUp]
    public void Setup()
    ...
    [Test]
    public void DrawCard_IsCardDrawFromTop()
    {   
        List<ICard> deck = new List<ICard>
        {
            {new Card(Suit.Clubs, Rank.Two)},
        ...
        ICard topCard = deck.First();
        _game.DrawCard();
        ICard newTopCard = deck.First();

        Assert.That(topCard != newTopCard, Is.False, "The card indeed draw from the top of deck!!" );
    }
}
