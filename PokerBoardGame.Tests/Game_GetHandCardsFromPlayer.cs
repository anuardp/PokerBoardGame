...
public class Game_GetHandCardsFromPlayer
{
    ...
    [Test]
    public void GetHand_GetCardsFromCurrentPlayerHands()
    {
        ...
        //Cek kartu dari pemain random
        IPlayer player = _game.GetCurrentPlayer();

        List<ICard> handCards = playerHands[player];
        Assert.That(handCards.Count == 2 && handCards[0] == playerHands[player][0] && handCards[1] == playerHands[player][1], Is.True, $"This is exactly the card from {player.Name}");
