...
[TestFixture]
public class Game_DealFlopDoesCardAddedOnBoard
{
    private Game _game;
    [SetUp]
    public void Setup()
    ...
    [Test]
    public void DealFlop_DealThreeCardsOnBoard()
    {
        ...
        while(phase != GamePhase.Flop)
        {
            _game.MoveToNextPhase();
            phase = _game.GetPhase();
        } 
        int prevCards = board.Count;
        _game.DealFlop();
        board = _game.GetBoard();
        int currentCards = board.Count;
        Assert.That(prevCards == 0 && currentCards == 3, Is.True, $"Tiga kartu sudah ditambah pada fase {phase}");   
    }
